...
#     clfs.append(clf)
#     from sklearn.externals import joblib
#     joblib.dump(clf, 'models/model-roulette-'+str(countModel)+'.pkl')
#     countModel+=1
#     # if percent > 54.15:
...
countLose = 0
converted = False
for i in range(len(y_test)-1,0,-1):
    y_pred[i] = convert_1_2(y_pred[i])
    # if countLose == 8 or countLose == 7:
    #     datas[0] = convert_1_2(datas[0])
    # if countLose == 1 and not converted:
    #     y_pred[i] = convert_1_2(y_pred[i])
    #     converted = True
    # if countLose == 1 and converted:
    #     converted = False
    if countLose == 11:
        y_pred[i] = convert_1_2(y_pred[i])
    if y_test[i] != y_pred[i]:
